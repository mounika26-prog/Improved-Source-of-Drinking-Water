"""
watsonx_notebook_adapter.py
============================
Adapter that makes the PS-38 analysis run inside IBM watsonx.ai Studio
without any changes to the core analysis logic.

This script is uploaded as a data asset alongside drinking_water_data.csv
and run as a Jupyter Notebook cell or a batch job inside watsonx.ai Studio.

What it does
------------
1. Downloads the CSV dataset from the watsonx.ai project's Cloud Object Storage
2. Runs the full analysis pipeline (same logic as validate_analysis.py)
3. Uploads all 11 output charts and 2 CSVs back to the project's COS bucket

Usage inside watsonx.ai Studio Notebook
----------------------------------------
    # Paste into the first cell of your notebook in watsonx.ai Studio:
    # Then click Run All

    # The notebook environment already has ibm-watson-studio-lib pre-installed.
    # pandas, numpy, matplotlib, seaborn are also pre-installed.

Requirements
------------
    ibm-watson-studio-lib  (pre-installed in watsonx.ai Studio notebooks)
    pandas, numpy, matplotlib, seaborn  (pre-installed in all runtimes)

Lite Plan Note
--------------
    This script uses ZERO foundation model tokens. It only uses notebook
    compute (CUH). The full analysis completes in ~2–3 minutes, using
    well under 1 CUH out of the 20 CUH/month free allowance.
"""
...
import warnings
import matplotlib
matplotlib.use('Agg')   # non-interactive backend — required inside watsonx runtimes
...
# ── 1. Connect to the watsonx.ai Studio project ──────────────────────────────
# ibm_watson_studio_lib is pre-installed in all watsonx.ai Studio notebooks.
# If running locally, install with: pip install ibm-watson-studio-lib
try:
    from ibm_watson_studio_lib import access_project_or_space
    # In watsonx.ai Studio, project_id and token are injected automatically.
    # When running locally, pass {"token": "<your_project_token>"}.
    wslib = access_project_or_space()
    RUNNING_IN_WATSONX = True
    print("✅  Connected to watsonx.ai Studio project via ibm_watson_studio_lib")
except Exception:
    RUNNING_IN_WATSONX = False
    print("ℹ️   Not running inside watsonx.ai Studio — using local file paths instead")

# ── 2. Load the dataset ────────────────────────────────────────────────────
if RUNNING_IN_WATSONX:
    # Download the CSV from the project's Cloud Object Storage bucket
    # The file must have been uploaded as a data asset in the project.
    wslib.download_file("drinking_water_data.csv")
    df = pd.read_csv("drinking_water_data.csv")
    print("✅  Loaded dataset from watsonx.ai project COS")
else:
    # Local fallback for development
    df = pd.read_csv("data/drinking_water_data.csv")
    print("✅  Loaded dataset from local path")

assert df.shape == (192, 14), f"Unexpected dataset shape: {df.shape}"
...
# ── Helper: save chart and optionally upload to watsonx COS ───────────────
def save_chart(fig: plt.Figure, filename: str) -> None:
    """Save chart to results/ and upload to watsonx project COS if available."""
    path = f"results/{filename}"
    fig.savefig(path, bbox_inches='tight')
    plt.close(fig)
    if RUNNING_IN_WATSONX:
        wslib.upload_file(path, filename, overwrite=True)
        print(f"  ✅  {filename} saved locally and uploaded to project COS")
    else:
        print(f"  ✅  {filename} saved to results/")


# ────────────────────────────────────────────────────────────────────────────
# CHART 1 — National Trend
# ────────────────────────────────────────────────────────────────────────────
print("\n📊 Generating Chart 1: National Trend...")
...
plt.tight_layout()
save_chart(fig, '01_national_trend.png')

# ────────────────────────────────────────────────────────────────────────────
# CHART 2 — Rural vs Urban Distribution
# ────────────────────────────────────────────────────────────────────────────
print("📊 Generating Chart 2: Rural–Urban Distribution...")
...
plt.tight_layout()
save_chart(fig, '02_rural_urban_distribution.png')

# ────────────────────────────────────────────────────────────────────────────
# CHART 3 — State Comparison 2021
# ────────────────────────────────────────────────────────────────────────────
print("📊 Generating Chart 3: State Comparison 2021...")
state_2021 = (
    df[df['Year'] == 2021]
...
plt.tight_layout()
save_chart(fig, '03_state_comparison_2021.png')

# ────────────────────────────────────────────────────────────────────────────
# CHART 4 — Regional Trends
# ────────────────────────────────────────────────────────────────────────────
print("📊 Generating Chart 4: Regional Trends...")
...
plt.tight_layout()
save_chart(fig, '04_regional_trends.png')

# ────────────────────────────────────────────────────────────────────────────
# CHART 5 — Top/Bottom States
# ────────────────────────────────────────────────────────────────────────────
print("📊 Generating Chart 5: Top & Bottom States...")
rural_2021 = (
...
ax.set_xlabel('Improved Water Access (%)'); ax.set_xlim(40, 105)
...
plt.tight_layout()
save_chart(fig, '05_top_bottom_states.png')
...
plt.tight_layout()
save_chart(fig, '06_correlation_heatmap.png')

# ────────────────────────────────────────────────────────────────────────────
# CHART 7 — Scatter Socioeconomic
# ────────────────────────────────────────────────────────────────────────────
print("📊 Generating Chart 7: Socio-Economic Scatter Plots...")
...
for ax, (xcol, xlabel, color) in zip(axes, scatter_pairs):
    ...
    ax.set_xlabel(xlabel, fontsize=9); ax.set_ylabel('Improved Water Access (%)', fontsize=9)
    ax.yaxis.set_major_formatter(mticker.PercentFormatter(xmax=100))
plt.suptitle('Scatter Analysis — Socio-Economic Drivers of Water Access',
             fontsize=13, fontweight='bold', y=1.02)
plt.tight_layout()
save_chart(fig, '07_scatter_socioeconomic.png')

# ────────────────────────────────────────────────────────────────────────────
# CHART 8 — Clean Fuel & Water
# ────────────────────────────────────────────────────────────────────────────
print("📊 Generating Chart 8: Clean Fuel & Water Access...")
...
axes[1].set_xlabel('Clean Cooking Fuel Access (%)'); axes[1].set_ylabel('Improved Water Access (%)')
axes[1].xaxis.set_major_formatter(mticker.PercentFormatter(xmax=100))
axes[1].yaxis.set_major_formatter(mticker.PercentFormatter(xmax=100))
axes[1].legend(title='Settlement')
plt.tight_layout()
save_chart(fig, '08_clean_fuel_water.png')

# ────────────────────────────────────────────────────────────────────────────
# CHART 9 — Migration Analysis
# ────────────────────────────────────────────────────────────────────────────
print("📊 Generating Chart 9: Migration Analysis...")
fig, axes = plt.subplots(1, 2, figsize=(13, 5))
for label in ['Rural', 'Urban']:
    subset = df[df['Rural_Urban'] == label]
...
plt.tight_layout()
save_chart(fig, '09_migration_analysis.png')

# ────────────────────────────────────────────────────────────────────────────
# CHART 10 — Improvement by State
# ────────────────────────────────────────────────────────────────────────────
print("📊 Generating Chart 10: Improvement by State...")
pivot = (
...
plt.tight_layout()
save_chart(fig, '10_improvement_by_state.png')

# ────────────────────────────────────────────────────────────────────────────
# CHART 11 — Fuel Pie Charts
# ────────────────────────────────────────────────────────────────────────────
print("📊 Generating Chart 11: Fuel Pie Charts...")
fuel_2021 = (
    df[df['Year'] == 2021]
...
plt.tight_layout()
save_chart(fig, '11_fuel_pie_charts.png')
...
if RUNNING_IN_WATSONX:
    wslib.upload_file('results/state_summary.csv', 'state_summary.csv', overwrite=True)
    wslib.upload_file('results/improvement_table.csv', 'improvement_table.csv', overwrite=True)
    print("  ✅  CSVs uploaded to project COS")
else:
...
print('=' * 60)
print('\n🎉  All charts and outputs generated successfully inside watsonx.ai!')
